...
func _http_request_completed(_result, _response_code, _headers, _body):
	...
	match lastCaller:
		"submitscore":
			 _get_score()
		"getscore":
			$ScoreDisplay.set_text(response['response']['score'])
	...
	if response['response']['size'] > 0:
		for n in (response['response']['size']):
			$"../ScoreDisplay".text = String(response['response'][String(0)]['username']) + "	" + String(response['response'][String(0)]['score'])
			$"../ScoreDisplay"
