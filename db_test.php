		...
		switch ($_REQUEST['command']){
			...
			case "add_score":
				...
				if (strlen($username) > 25)
					$username = substrt($username, 25);
				
				$score = $dict_from_json['score'];
				
				$template = "INSERT INTO `highscore` (username, score) VALUES (:username, :score) ON DUPLICATE
            ...
            case "get_score":
				...
				$template = "SELECT score FROM `highscore` where username = :username;";  //" ORDER BY score DESC LIMIT 1;";
				...
				$player = $sth -> fetchAll(PDO::FETCH_ASSOC);

				CloseConnPDO($pdo);
            	print_response("",[],"get_score");
				print_response("highscore", $player);
			...
			*/
				...
				$template = "SELECT * FROM `highscore` ORDER BY `highscore`.`score` DESC";
				#Make a connection to the DB
				$pdo = OpenConnPDO();
				//verify_nonce($pdo);
				if(!verify_nonce($pdo))
